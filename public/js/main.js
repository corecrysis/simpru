...
$(document).ready(function () {
    ...
    (function ($) {
        ...
        $('#home-2-slideshow .mv-slider-wrapper').carouFredSel({
            ...
            prev: {
                button: '#home-2-slideshow-prev'
            },

            next: {
                button: '#home-2-slideshow-next'
            }
        });
    })(jQuery);
    ...
    /* --------------------------------------------------------------------- */
    (function ($) {
        $('.home-3-logo-brand-slider').each(function () {
            if (!$(this).length) return;
        });

        $('.home-3-logo-brand-slider .mv-slider-wrapper').each(function () {
            $(this).carouFredSel({
                infinite: true,
                circular: true,
                responsive: true,
                ...
                prev: {
                    button: $(this).data('prev')
                },

                next: {
                    button: $(this).data('next')
                }
            });
        });
    })(jQuery);
    ...
    (function ($) {
        ...
        $('.mv-slider-range').each(function () {
            ...
            var minInputValue = wrapper.find('.input-min-value');
            var maxInputValue = wrapper.find('.input-max-value');
            ...
            sliderRange.slider({
                ...
                slide: function (event, ui) {
                    ...
                    minInputValue.val(ui.values[0]);
                    maxInputValue.val(ui.values[1]);
                }
            });

            minValue.text(sliderRange.slider('values', 0));
            maxValue.text(sliderRange.slider('values', 1));
            minInputValue.val(sliderRange.slider('values', 0));
            maxInputValue.val(sliderRange.slider('values', 1));
        });
    })(jQuery);
    ...
    (function ($) {
        ...
        $(document.body).on('click', '.btn-delete-product', function () {
            ...
            if(i){
                ...
                $.ajax({
                    type: "DELETE",
                    url: host + '/booking/hapusBook/' + rowId,
                    success: function (data) {
                        row.closest('.post').fadeOut(300, function () {
                            row.remove();
                            $('.count-cart').html(data.items.total_book);
                        });
